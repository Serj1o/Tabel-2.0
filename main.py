...
class WorkTimeBot:
    ...
    async def show_stats(self, callback: types.CallbackQuery):
        ...
        sick_leaders.sort(key=lambda r: int(row['sick_days'] or 0), reverse=True)
        non_working = [employee_names[emp['id']] for emp in employees if emp['id'] not in worked_ids]
        ...
        if not top_hours:
            ...
            for row in top_hours:
                text += f"• {employee_names.get(row['employee_id'], '—')} — {float(row['hours'] or 0):.1f} ч, дней: {int(row['days_worked'] or 0)}\n"
        ...
        if not sick_leaders:
            ...
            for row in sick_leaders:
                text += f"• {employee_names.get(row['employee_id'], '—')} — {int(row['sick_days'] or 0)} дн.\n"
